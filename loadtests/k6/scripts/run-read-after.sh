#!/bin/bash
# ============================================================
# Week2 · 캐시 적용 후(AFTER) 조회 폭주 측정
# ----------------------------------------------------------
# 캐시 ON 상태에서 조회 p99 / RPS / health 지연을 수집한다.
# BEFORE 와 "동일한 RATE/DURATION"으로 돌려야 비교가 성립한다.
# 정확성 게이트(read_failed_rate)는 항상 적용된다(에러 벤치마킹 방지). 기본은 p99 합격선 미적용.
# p99 합격선(캐시 효과)까지 판정하려면:
#   USE_THRESHOLDS=1 READ_P99_MS=200 ./scripts/run-read-after.sh
# (p99 기준은 config/env.js readThresholds = read_latency_ms p(99)<READ_P99_MS 참고)
# 본 측정 전에 preflight smoke 로 조회 동작을 먼저 확인하고, 실패하면 벤치마크를 돌리지 않는다.
#
# 사용:
#   ./scripts/run-read-after.sh                 # 결과 파일명 자동(timestamp)
#   ./scripts/run-read-after.sh my-after-tag    # 결과 파일명 접두 인자
# 강도/대상 조절:
#   RATE=300 DURATION=2m TARGET_PRODUCT=1 COLD_RATIO=0.1 ./scripts/run-read-after.sh
# ============================================================
...
MODE="${MODE:-steady}"
USE_THRESHOLDS="${USE_THRESHOLDS:-0}"
...
echo "============================================================"
echo "[READ AFTER START] RATE=$RATE DURATION=$DURATION TARGET=$TARGET_PRODUCT COLD_RATIO=$COLD_RATIO MODE=$MODE USE_THRESHOLDS=$USE_THRESHOLDS"
echo "BASE_URL=$BASE_URL"
echo "============================================================"

# 정확성 게이트(read_failed_rate)는 항상 적용. USE_THRESHOLDS=1 이면 -e USE_THRESHOLDS=1 로
# JUDGE 모드가 켜져 p99 합격선(read_latency_ms)까지 추가로 강제된다.
k6 run \
    -e RATE="$RATE" \
    -e DURATION="$DURATION" \
    -e TARGET_PRODUCT="$TARGET_PRODUCT" \
    ...
    -e MEMBER_MAX="$MEMBER_MAX" \
    -e PRODUCT_MAX="$PRODUCT_MAX" \
    -e READ_P99_MS="${READ_P99_MS:-200}" \
    -e USE_THRESHOLDS="$USE_THRESHOLDS" \
    --out "json=$RESULT_FILE" \
    --summary-export="$SUMMARY_FILE" \
    "$SCRIPT"

echo "============================================================"
echo "[READ AFTER STOP]"
echo "결과: $RESULT_FILE"
