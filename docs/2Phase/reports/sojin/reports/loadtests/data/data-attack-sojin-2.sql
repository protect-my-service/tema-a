...
--    ordered_at = (NOW() AT TIME ZONE 'Asia/Seoul') - 10분
--      ※ DB 컨테이너 timezone 이 UTC 라도, 방어 서버 JVM(KST)의
--        LocalDateTime.now() 와 wall-clock 을 맞추기 위해 KST 로 강제 변환.
...
SELECT
    ...
    (NOW() AT TIME ZONE 'Asia/Seoul') - INTERVAL '10 minutes'
...
--    paid_at = ordered_at + 1분 (KST wall-clock 으로 통일)
...
SELECT
    ...
    110000.00,
    'APPROVED',
    (NOW() AT TIME ZONE 'Asia/Seoul') - INTERVAL '9 minutes'
FROM orders o
WHERE o.order_number LIKE 'ORD-A05-%';
